...
public class AdminAfficherRdv {
    ...
    @FXML
    private Label rendezVousInfoLabel;

    public void initData(RendezVous rdv) {
        String info = "Date: " + rdv.getDate() + "\nType: " + rdv.getType(); // Customize according to your needs
        ...
        try {
            ...
            tableViewRdv.setOnMouseClicked(event -> {
                if (event.getClickCount() == 1) {
                    ...
                    if (selectedRdv != null) {
                        try {
                            FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/example/vitalize/AdminEditerRdv.fxml"));
                            Parent root = loader.load();
                            AdminEditerRdv controller = loader.getController();
    ...
    void NaviguerRdv(ActionEvent event) {
        try {
            Parent root = FXMLLoader.load(getClass().getResource("/AdminRechercheDoctor.fxml"));
            Scene scene = new Scene(root);

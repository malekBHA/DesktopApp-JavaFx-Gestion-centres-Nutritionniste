...
public class AdminAfficherFiche {
    @FXML
            ...
            TableViewFiche.setOnMouseClicked(event -> {
                if (event.getClickCount() == 1) { // Detect double click
                    ...
                    if (selectedFichePatient != null) {
                        try {
                            FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/example/vitalize/AdminEditerFiche.fxml"));
                            Parent root = loader.load();
                            AdminEditerFiche controller = loader.getController();
    ...
    void CreateNewFormBtn(ActionEvent event2) {
        try {
            Parent root = FXMLLoader.load(Objects.requireNonNull(getClass().getResource("/com/example/vitalize/AdminAjouterFiche.fxml")));
            Scene scene = new Scene(root);
    ...
    void NavToRdv(ActionEvent event4) {
        try {
            Parent root = FXMLLoader.load(getClass().getResource("/com/example/vitalize/AdminRendezVous.fxml"));
            Scene scene = new Scene(root);
    ...
    private void generateExcel(FichePatient fichePatient) {
        ...
        Sheet sheet = workbook.createSheet("AdminFichePatient");
        ...
        try {
            int fileNumber = 1;
            String fileName = "D:\\ Admin FichePatient" + fileNumber + ".xlsx";
            File file = new File(fileName);
            while (file.exists()) {
                fileNumber++;
                fileName = "D:\\Admin FichePatient" + fileNumber + ".xlsx";
            ...
            fileOut.close();
            workbook.close();
            System.out.println("Admin Excel file generated successfully: " + fileName);

            Alert alert = new Alert(Alert.AlertType.INFORMATION);
            alert.setTitle("Success");
            alert.setHeaderText(null);
            alert.setContentText("Admin Excel file generated successfully: " + fileName);
            alert.showAndWait();
        } catch (Exception e) {
            System.err.println("Error occurred while generating Admin Excel file: " + e.getMessage());
        }
    }
    ...
    private void generatePdf(FichePatient fichePatient) {
        try {
            int fileNumber = 1;
            String fileName = "D:\\Admin FichePatient" + fileNumber + ".pdf";
            File file = new File(fileName);
            while (file.exists()) {
                fileNumber++;
                fileName = "D:\\Admin FichePatient" + fileNumber + ".pdf";
                file = new File(fileName);
            }
